...
ind_shift = len(extremum_dimensional_xt)-10 #manual shift from begining
...
for ind in enumerate(extremum_dimensional_xt):
    if ind[0]>2+ind_shift:
        ...
        if len(error_array_a)>3 :
            if abs(error_array_a[-1]-error_array_a[-2])/np.average(error_array_a[0:-1]) > 0.5:
                cutoff_index = ind[0]-1
                del error_array_a[-1]
                del error_array_b[-1]
                break
        if len(error_array_b)>3 :
            if abs(error_array_b[-1]-error_array_b[-2])/np.average(error_array_b[0:-1]) > 0.5:
                cutoff_index = ind[0]-1
                del error_array_a[-1]
                del error_array_b[-1]
                break
